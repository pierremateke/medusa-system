...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('team-derank')
        .setDescription('» Teamupdate für einen Team-Derank erstellen.')
        .addUserOption(option =>
            option.setName('user')
                .setDescription('Das Teammitglied, das degradiert wird.')
                .setRequired(true))
        ...
        .addStringOption(option =>
            option.setName('grund')
                .setDescription('Der Grund für den Derank.')
                .setRequired(true)),
    async execute(interaction) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setTitle('DERANK')
                .setDescription(
                    `**${targetUser}** wurde im Team degradiert!\n\n` +
                    `**Von:** <@&${oldRole.id}>\n` +
                    `**Auf:** <@&${newRole.id}>\n` +
                    `**Grund:** \`${reason}\`\n` +
                    `**Am:** <t:${nowUnix}:f>\n\n` +
            ...
            await interaction.reply({ content: 'Der Team-Derank wurde erfolgreich verkündet und die Rollen wurden angepasst.', ephemeral: true });
        } catch (error) {
