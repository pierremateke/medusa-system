...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('frak-aufgelöst')
        .setDescription('» Verkündet die Auflösung einer Fraktion.')
        .addStringOption(option =>
            option.setName('fraktion')
                .setDescription('Der Name der Fraktion, die aufgelöst wird.')
                .setRequired(true))
        .addStringOption(option =>
            option.setName('grund')
                .setDescription('Der Grund für die Auflösung.')
                .setRequired(true)),
    async execute(interaction) {
        try {
            ...
            await interaction.reply({ content: 'Die Fraktionsauflösung wurde erfolgreich verkündet.', ephemeral: true });
        } catch (error) {
